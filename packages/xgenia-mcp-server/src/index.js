...
async function getPage() {
  ...
  if (!browser || !browser.isConnected()) {
    try {
      // Electron apps with --remote-debugging-port=9223 expose CDP here
      browser = await chromium.connectOverCDP('http://127.0.0.1:9223');
    } catch (e) {
      throw new Error(`Failed to connect to XGENIA on port 9223. Ensure XGENIA is running with --remote-debugging-port=9223. Error: ${e}`);
    }
  }
  ...
  // Find a page with actual dimensions (filtering out background/hidden pages)
  for (const p of pages) {
    ...
    try {
      ...
      if (!isHidden && dims.w > 0 && dims.h > 0) {
        page = p;
        return page;
      }
    } catch (e) {
      // Ignore evaluate errors on some internal pages
    }
  }
