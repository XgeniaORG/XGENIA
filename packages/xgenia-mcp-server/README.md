...
| Tool | What it does |
| --- | --- |
| `xgenia_health` | Liveness: running or not, dev or packaged, which project is open, whether the AI chat panel is mounted, how long it's been generating, whether anyone is signed in (`authenticated`), and the panel's active model, running cost and context usage (`chatModel`, `chatCost`, `chatContextUsage`). Never throws on a dead or wedged editor — reports `{running: false, code, hint, rawCdp}` instead, with `code` distinguishing `not-running`, `editor-unresponsive` (the editor page did not answer a raw CDP probe) and `connect-stalled` (the editor page answered; only Playwright's attach stalled — retry, don't force-restart). Call this first. |
...
| `xgenia_open_project` | Open a project by absolute directory or by name. Saves whatever is open, appends the recents entry, then reloads the lobby — the only moment it re-reads the recents file — and clicks the tile. Verifies the editor actually landed on it, and reports which state the page was actually in (login screen, empty projects screen, or a different project open) on timeout. |
...
| `xgenia_chat_read` | Read the AI chat transcript, paged from an index, plus `model`, `cost` and `contextUsage` from the panel header/footer. An empty conversation is `total: 0`, not a blob of panel chrome. |
| `xgenia_preview_read` | Read the running game first-hand: visible text of the preview or of one node (by editor `label` or CSS `selector`), plus every rendered node carrying `data-xgenia-node-label` with its current text and visibility. Straight from the preview DOM, not the panel AI's account of it. |
| `xgenia_preview_click` | Click a rendered node by `label` (or `selector`) `times` times and report another node's text (`readLabel`) before and after each press. |
| `xgenia_chat_wait_idle` | Block until the AI chat panel stops generating; returns `timedOut` instead of throwing. |
| `xgenia_screenshot` | Capture the editor window, the chat panel, or the canvas. Returns `cssSize`, `imageSize`, `scale`, `contentSize` and a `note` when the buffer is padded or cropped by the Electron zoom factor. |
