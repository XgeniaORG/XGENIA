...
const MaxNode = {
  ...
  category: 'Math',
  color: 'data',
  searchTags: ['max', 'maximum', 'larger', 'math', 'number'],
  initialize: function () {
  ...
  getInspectInfo() {
    ...
    return {
      type: 'value',
      value: `max(${this._internal.firstNumber}, ${this._internal.secondNumber}) = ${this._internal.result}`
    };
  },
  inputs: {
    Do: {
    ...
    firstNumber: {
      ...
      set: function (value) {
        try {
          ...
          console.error('Max Node - First Number error:', error.message);
        }
      }
    },
    secondNumber: {
      ...
      set: function (value) {
        try {
          ...
          this._internal.lastError = error.message;
          console.error('Max Node - Second Number error:', error.message);
        }
      }
    }
  },
