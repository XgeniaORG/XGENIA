...
var ConfigNodeDefinition = {
  name: 'DbConfig',
  docs: 'https://docsapp.xgenia.com/nodes/data/cloud-data/config',
  ...
  initialize: function () {
    ...
    ConfigService.instance
      .getConfig()
      .then((config) => {
        console.log('[DbConfig] Config:', config);
        internal.config = config;
        if (this.hasOutput('value')) this.flagOutputDirty('value');
      })
      .catch((error) => {
        console.warn('[DbConfig] Failed to load config:', error);
        // Set empty config to prevent undefined errors
        internal.config = {};
        if (this.hasOutput('value')) this.flagOutputDirty('value');
      });
  },
...
module.exports = {
  ...
  setup: function (context, graphModel) {
    ...
    function updatePorts(node) {
      ...
      if (configSchema) {
        const isCloud = typeof _xgenia_cloud_runtime_version !== 'undefined';
        ports.push({
          name: 'configKey',
