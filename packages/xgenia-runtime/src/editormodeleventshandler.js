...
async function handleEvent(context, graphModel, event) {
    ...
    var eventHandlers = {
        nodeAdded: async function(event) {
            await componentModel.importEditorNodeData(event.model, event.parentId, event.childIndex);
        },
        ...
        componentAdded: async function(event) {
            await graphModel.importComponentFromEditorData(event.model);
        },
