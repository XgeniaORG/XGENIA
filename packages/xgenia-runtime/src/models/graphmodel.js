...
GraphModel.prototype.importComponentFromEditorData = async function (componentData) {
  console.log('[GraphModel] importComponentFromEditorData called with:', componentData.name);
  
  // Check if componentData has the hierarchical graph structure
  if (componentData.graph && componentData.graph.roots) {
    console.log('[GraphModel] Converting hierarchical graph structure to flat structure');
    
    // Flatten the hierarchical structure
    const flattenedData = this._flattenComponentData(componentData);
    var componentModel = await ComponentModel.createFromExportData(flattenedData);
  } else {
    // Use data as-is if already in flat structure
    console.log('[GraphModel] Using component data as-is (already flat structure)');
...
// Helper method to flatten hierarchical component data
GraphModel.prototype._flattenComponentData = function (componentData) {
  const nodes = [];
  const rootIds = [];
  
  // Recursive function to flatten node hierarchy
  const flattenNode = (node, parentId = null) => {
    // Add the node to the flat array
    const flatNode = { ...node };
    delete flatNode.children; // Remove children property from flat node
    nodes.push(flatNode);
    
    // If this is a root node (no parent), add to rootIds
    if (!parentId) {
  ...
  // Flatten all root nodes and their children
  if (componentData.graph.roots && Array.isArray(componentData.graph.roots)) {
    for (const root of componentData.graph.roots) {
      flattenNode(root);
    }
  }
  
  console.log(`[GraphModel] Flattened ${componentData.name}: ${nodes.length} nodes, ${rootIds.length} roots`);
  
  // Return flattened structure that ComponentModel expects
  return {
    name: componentData.name,
    nodes: nodes,
    connections: componentData.graph.connections || [],
    roots: rootIds,
...
GraphModel.prototype.importEditorData = async function (exportData) {
  console.log('[GraphModel] importEditorData called with exportData:', exportData);
  
  this.componentIndex = exportData.componentIndex;
  this.routerIndex = exportData.routerIndex;
  
  console.log('[GraphModel] Set routerIndex to:', this.routerIndex);
  ...
  if (exportData.components && Array.isArray(exportData.components)) {
  ...
  let rootComponent = exportData.rootComponent;
  
  if (!rootComponent && exportData.components && exportData.components.length > 0) {
    console.log('[GraphModel] No rootComponent specified, determining from project structure...');
    
    // Look for component containing Router node
    for (const component of exportData.components) {
      if (component.graph && component.graph.roots) {
        const hasRouter = this._componentHasRouter(component.graph.roots);
        if (hasRouter) {
          rootComponent = component.name;
          console.log('[GraphModel] Found root component with Router:', rootComponent);
          break;
        }
      }
    }
    
    // Fallback to first component if no Router found
    if (!rootComponent) {
      rootComponent = exportData.components[0].name;
      console.log('[GraphModel] Using first component as root:', rootComponent);
    }
  }

  console.log('[GraphModel] Setting root component to:', rootComponent);
  this.setRootComponentName(rootComponent);
};

// Helper method to check if component has Router node
GraphModel.prototype._componentHasRouter = function (roots) {
  for (const root of roots) {
    ...
    if (root.children && root.children.length > 0) {
      for (const child of root.children) {
        ...
        // Could recursively check deeper if needed
      }
    }
  }
  return false;
