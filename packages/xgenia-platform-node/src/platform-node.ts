...
function getAppPath() {
  ...
  throw `[@xgenia/platform] Cannot find package.json, to get the build version. (${__dirname})`;
}
...
export class PlatformNode implements IPlatform {
  ...
  constructor() {
    ...
    this._userDataPath = path.join(roamingPath, 'XGENIA');
    ...
    if (!fs.existsSync(packagePath)) {
      throw `[@xgenia/platform] Cannot find package.json, to get the build version. ('${packagePath}')`;
    }
