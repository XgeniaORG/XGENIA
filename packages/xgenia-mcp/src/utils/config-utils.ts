import { DEFAULT_MCP_PROXY_LISTEN_PORT, DEFAULT_INSPECTOR_CONFIG } from '../lib/constants';
import { InspectorConfig } from './configuration-types';
...
export const getInitialTransportType = (): 'stdio' | 'sse' | 'streamable-http' => {
  const param = getSearchParam('transport');
  if (param === 'stdio' || param === 'sse' || param === 'streamable-http') {
    return param;
  }
  return (localStorage.getItem('lastTransportType') as 'stdio' | 'sse' | 'streamable-http') || 'stdio';
};
...
export const getConfigOverridesFromQueryParams = (defaultConfig: InspectorConfig): Partial<InspectorConfig> => {
  const url = new URL(window.location.href);
...
export const initializeInspectorConfig = (localStorageKey: string): InspectorConfig => {
  ...
  for (const [key, value] of Object.entries(baseConfig)) {
    baseConfig[key as keyof InspectorConfig] = {
      ...
      description: DEFAULT_INSPECTOR_CONFIG[key as keyof InspectorConfig].description,
      is_session_item: DEFAULT_INSPECTOR_CONFIG[key as keyof InspectorConfig].is_session_item
    };
  }
...
export const saveInspectorConfig = (localStorageKey: string, config: InspectorConfig): void => {
