...
// Import schemas for multimedia AI tools
import { GenerateImageArgsSchema } from './tools/generate_image.js';
import { GenerateVideoArgsSchema } from './tools/generate_video.js';
import { AnalyzeEditImageArgsSchema } from './tools/analyze_edit_image.js';
import { CheckApiSetupArgsSchema } from './tools/check_api_setup.js';
import { BatchImageOperationsArgsSchema } from './tools/batch_image_operations.js';
import { ConfigureGoogleCloudArgsSchema } from './tools/configure_google_cloud.js';
import { ConfigureAISettingsArgsSchema } from './tools/configure_ai_settings.js';
import { ConfigurePrioritySettingsArgsSchema } from './tools/configure_priority_settings.js';

// Import multimedia client functions
import { 
    generateImageWithVertexAI, 
    generateVideoWithVertexAI, 
    analyzeImageWithVertexAI, 
    checkAPISetup, 
    performBatchImageOperations,
    handleCredentialInput
} from './multimedia_client.js';
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (toolName === "save_generate_project_guidelines") {
        ...
        const parsedArgs = GenerateImageArgsSchema.parse(args);
        const { 
            prompt, 
            output_path, 
            width, 
        ...
        try {
            // Actually generate the image using Vertex AI
            const imageBuffer = await generateImageWithVertexAI(
                prompt,
                width,
                height,
                quality,
            ...
            const validOutputPath = validateWorkspacePath(output_path);
            await fs.mkdir(path.dirname(validOutputPath), { recursive: true });
            await fs.writeFile(validOutputPath, imageBuffer);

            return {
                content: [{ 
                    type: "text", 
                    text: `✅ Successfully generated image and saved to ${output_path}\n\nGenerated with:\n- Prompt: ${prompt}\n- Resolution: ${width}x${height}\n- Quality: ${quality}\n- Style: ${style_preset || 'photographic'}\n- Guidance Scale: ${guidance_scale}${seed ? `\n- Seed: ${seed}` : ''}` 
                }],
            };
        } catch (error: any) {
        ...
        const parsedArgs = GenerateVideoArgsSchema.parse(args);
        const { 
            prompt, 
            output_path, 
            duration, 
        ...
        try {
            // Actually generate the video using Vertex AI
            const videoBuffer = await generateVideoWithVertexAI(
                prompt,
                duration,
                width,
                height,
            ...
            const validOutputPath = validateWorkspacePath(output_path);
            await fs.mkdir(path.dirname(validOutputPath), { recursive: true });
            await fs.writeFile(validOutputPath, videoBuffer);

            return {
                content: [{ 
                    type: "text", 
                    text: `✅ Successfully generated video and saved to ${output_path}\n\nGenerated with:\n- Prompt: ${prompt}\n- Duration: ${duration}s\n- Resolution: ${width}x${height}\n- FPS: ${fps}\n- Style: ${style_preset || 'realistic'}\n- Motion: ${motion_intensity}\n- Camera: ${camera_movement}${seed ? `\n- Seed: ${seed}` : ''}` 
                }],
            };
        } catch (error: any) {
            if (error instanceof McpError) {
    ...
    } else if (toolName === "analyze_edit_image") {
        const parsedArgs = AnalyzeEditImageArgsSchema.parse(args);
        const { 
            image_path, 
            analysis_type, 
            edit_request, 
        ...
        try {
            // Actually analyze the image using Vertex AI Vision
            const analysisResult = await analyzeImageWithVertexAI(
                image_path,
                analysis_type,
                focus_areas,
                comparison_reference
            ...
            if (output_path) {
                const validOutputPath = validateWorkspacePath(output_path);
                await fs.mkdir(path.dirname(validOutputPath), { recursive: true });
                await fs.writeFile(validOutputPath, analysisResult, "utf-8");
                return {
                    content: [{ 
                        type: "text", 
                        text: `✅ Image analysis complete and results saved to ${output_path}\n\nAnalysis Type: ${analysis_type}\nImage: ${image_path}${focus_areas && focus_areas.length > 0 ? `\nFocus Areas: ${focus_areas.join(', ')}` : ''}${comparison_reference ? `\nComparison: ${comparison_reference}` : ''}` 
                    }],
                };
            } else {
                return {
    ...
    } else if (toolName === "check_api_setup") {
        const parsedArgs = CheckApiSetupArgsSchema.parse(args);
        const { service_type, include_instructions, check_permissions } = parsedArgs;

        try {
            // Actually check the API setup
            const setupStatus = await checkAPISetup(service_type);

            return {
                content: [{ 
                    type: "text", 
                    text: `🔧 API Setup Check Results:\n\n${setupStatus}` 
                }],
            };
        } catch (error: any) {
            if (error instanceof McpError) {
            ...
            throw new McpError(ErrorCode.InternalError, `API setup check failed: ${error.message}`);
        }

    } else if (toolName === "configure_google_cloud") {
        const parsedArgs = ConfigureGoogleCloudArgsSchema.parse(args);
        const { project_id, location, auto_retry } = parsedArgs;

        try {
            // Handle the credential configuration
            const result = await handleCredentialInput(project_id, location);
            ...
            throw new McpError(ErrorCode.InternalError, `Credential configuration failed: ${error.message}`);
        }

    } else if (toolName === "configure_ai_settings") {
        const parsedArgs = ConfigureAISettingsArgsSchema.parse(args);
        const { tab, model_id, temperature, max_tokens, quality, style_preset, enable_streaming, auto_retry } = parsedArgs;

        try {
            // Handle the AI settings configuration
            const result = `🎛️ AI Settings configured for ${tab}:\n\n` +
                          `✅ Tab: ${tab}\n` +
                          ...
                          `${enable_streaming !== undefined ? `✅ Streaming: ${enable_streaming ? 'Enabled' : 'Disabled'}\n` : ''}` +
                          `\nYour AI settings have been updated! The system will use these settings for ${tab} operations.`;
            ...
            throw new McpError(ErrorCode.InternalError, `AI settings configuration failed: ${error.message}`);
        }

    } else if (toolName === "configure_priority_settings") {
        const parsedArgs = ConfigurePrioritySettingsArgsSchema.parse(args);
        const { capability, priority_order, default_model, fallback_models, quality_threshold, auto_fallback, cost_optimization, region_preference, custom_weights } = parsedArgs;

        try {
            // Handle the priority settings configuration
            const result = `🎯 Priority Settings configured for ${capability}:\n\n` +
                          `✅ Capability: ${capability}\n` +
                          `✅ Priority Order: ${priority_order.join(' → ')}\n` +
                          `✅ Default Model: ${default_model}\n` +
                          `${fallback_models ? `✅ Fallback Models: ${fallback_models.join(', ')}\n` : ''}` +
                          `✅ Quality Threshold: ${quality_threshold}\n` +
                          `✅ Auto Fallback: ${auto_fallback ? 'Enabled' : 'Disabled'}\n` +
                          `✅ Cost Optimization: ${cost_optimization ? 'Enabled' : 'Disabled'}\n` +
                          `${region_preference ? `✅ Region Preference: ${region_preference.join(', ')}\n` : ''}` +
                          `${custom_weights ? `✅ Custom Weights: ${JSON.stringify(custom_weights)}\n` : ''}` +
                          `\nYour priority settings have been updated! The system will now use these settings to intelligently select the best models for ${capability} operations.`;
            ...
            throw new McpError(ErrorCode.InternalError, `Priority settings configuration failed: ${error.message}`);
        }

    } else if (toolName === "batch_image_operations") {
        const parsedArgs = BatchImageOperationsArgsSchema.parse(args);
        const { 
            operation_type, 
            input_directory, 
            output_directory, 
        ...
        try {
            // Actually perform the batch operations
            const batchResults = await performBatchImageOperations(
                operation_type,
                input_directory,
                output_directory,
                operation_params,
            ...
            const validOutputPath = validateWorkspacePath(output_directory);
            await fs.mkdir(validOutputPath, { recursive: true });
            
            // Save the batch operation results
            const resultsPath = path.join(validOutputPath, 'batch_operation_results.txt');
            return {
                content: [{ 
                    type: "text", 
                    text: `🔄 Batch Image Operations Complete!\n\nResults saved to: ${resultsPath}\n\nOperation Summary:\n${batchResults}` 
                }],
            };
        } catch (error: any) {
            if (error instanceof McpError) {
