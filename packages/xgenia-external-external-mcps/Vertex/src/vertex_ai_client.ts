...
// Initialize AI client lazily - only when needed for actual API calls
...
function initializeAI(): void {
    ...
    try {
        if (aiConfig.geminiApiKey) {
            ...
            console.log("Initialized GoogleGenAI with Gemini API key, model:", aiConfig.modelId);
            ...
            console.log("Initialized GoogleGenAI with Vertex AI credentials, model:", aiConfig.modelId);
        } else {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to initialize AI client: ${error.message}`);
    }
}
...
export async function callGenerativeAI(
    ...
    if (!ai) {
        throw new McpError(ErrorCode.InternalError, "AI client failed to initialize");
    }
