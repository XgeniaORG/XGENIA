...
// --- Validation ---
// Note: We don't exit here anymore - the server can start in placeholder mode
// and credentials can be set dynamically through the UI
if (AI_PROVIDER === "vertex" && !GCLOUD_PROJECT) {
  console.warn("Warning: AI_PROVIDER is 'vertex' but GOOGLE_CLOUD_PROJECT environment variable is not set.");
  console.warn("The server will start in placeholder mode. Users can configure credentials through the UI.");
}

if (AI_PROVIDER === "gemini" && !GEMINI_API_KEY) {
  console.warn("Warning: AI_PROVIDER is 'gemini' but GEMINI_API_KEY environment variable is not set.");
  console.warn("The server will start in placeholder mode. Users can configure credentials through the UI.");
}
