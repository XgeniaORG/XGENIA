...
// --- Inference Client ---
let hfInferenceInstance = null;
export function getHfInference() {
    if (hfInferenceInstance)
...
export async function listEndpoints(namespace) {
    const config = getConfig();
...
export async function deleteEndpoint(namespace, name) {
    const config = getConfig();
    if (!config.accessToken) {
...
export async function getEndpointStatus(namespace, name) {
    const config = getConfig();
    if (!config.accessToken) {
