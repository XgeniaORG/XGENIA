...
export const create_endpoint = {
    ...
    inputSchema: zodToJsonSchema(CreateEndpointArgsSchema) as any,
    handler: async (args: z.infer<typeof CreateEndpointArgsSchema>) => {
        try {
            const payload = {
                name: args.name,
            ...
            const result = await createEndpoint(payload as any);
