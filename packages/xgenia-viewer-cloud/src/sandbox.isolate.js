...
_xgenia_api_response = (token,res) => {
...
const _defineAPI = () => {
    ...
    global.console = {
        log:function() {
            ...
            _xgenia_api_call('log',undefined,{level:'info',message:text})
        },
        info:function() {
        ...
        error:function() {
            ...
            _xgenia_api_call('log',undefined,{level:'error',message:text})
        }
    }
    ...
    global.fetch = async function(url,args) {
        return new Promise((resolve,reject) => {
            ...
            _xgenia_api_call("fetch",token,{url,...args})
        })
    }
    ...
    global.setTimeout = function(cb,millis) {
        ...
        _xgenia_api_call("setTimeout",token,millis)
    }

    global.setImmediate = function(cb) {
        eventQueue.push(cb)
        _xgenia_request_process_jobs()
    }
}
...
_xgenia_handleReq = (token,req) => {
    ...
    handleRequest(req).then(r => {
        ...
        _xgenia_response(token,r)
    }).catch(e => {
        console.error(`Cloud function ${req.function} response [400] message: ${e.message} (requestId:${token})`)
        _xgenia_response(token,{statusCode:400,body:JSON.stringify({error:e.message})})
    })
}