...
const CloudStore = require('@xgenia/runtime/src/api/cloudstore'),
  JavascriptNodeParser = require('@xgenia/runtime/src/javascriptnodeparser'),
  QueryUtils = require('@xgenia/runtime/src/api/queryutils');

var AggregateNode = {
  name: 'xgenia.cloud.aggregate',
  docs: 'https://docsapp.xgenia.com/nodes/cloud-functions/cloud-data/aggregate-records',
...
module.exports = {
  ...
  setup: function (context, graphModel) {
    ...
    graphModel.on('editorImportComplete', () => {
      graphModel.on('nodeAdded.xgenia.cloud.aggregate', function (node) {
        _managePortsForNode(node);
      });

      for (const node of graphModel.getNodesWithType('xgenia.cloud.aggregate')) {
        _managePortsForNode(node);
      }
    });
  }
