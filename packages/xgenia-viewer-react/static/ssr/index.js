...
globalThis.__xgenia_modules = [];
globalThis.XGENIA = {
  defineModule: function (m) {
    globalThis.__xgenia_modules.push(m);
  },
  deployed: true
};
...
globalThis.fetch = async (args) => {
  if (typeof args === 'string') {
    ...
    if (args.startsWith('/xgenia_bundles') && fs.existsSync(relativePath)) {
...
// Import the XGENIA runtime
require('./xgenia.deploy');

// From that file we get some runtime stuff defined on "XgeniaSSR"
const { createElement, ssrSetupRuntime } = globalThis.XgeniaSSR;
...
async function buildPage(path) {
  return new Promise((resolve) => {
    const xgeniaModules = globalThis.__xgenia_modules;
    ...
    const ViewerComponent = createElement(xgeniaModules, projectData);
    log('created.');

    const xgeniaRuntime = ViewerComponent.props.xgeniaRuntime;

    xgeniaRuntime.eventEmitter.on('SSR_PageLoading', (id) => {
      console.log('SSR_PageLoading', id);
    });

    xgeniaRuntime.eventEmitter.on('SSR_PageReady', (id) => {
      console.log('SSR_PageReady', id);
    });

    xgeniaRuntime.eventEmitter.on('rootComponentUpdated', async () => {
      log('Spin up...');
      xgeniaRuntime.rootComponent.triggerDidMount();
      for (let index = 0; index < 1000; index++) {
        await new Promise((resolve) => setImmediate(() => resolve(), 0));
        xgeniaRuntime.rootComponent.triggerDidMount();
        xgeniaRuntime._doUpdate();
      }
      log('done.');
    ...
    log('Setup Runtime...');
    ssrSetupRuntime(xgeniaRuntime, xgeniaModules, projectData);
    log('done.');
  });
}
