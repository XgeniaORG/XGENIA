...
export interface TextInputProps extends XGENIA.ReactProps {
  id: string;
...
type InputRef = ((HTMLTextAreaElement | HTMLInputElement) & {
  xgeniaNode?: XGENIA.ReactProps['xgeniaNode'];
}) | null;
...
export class TextInput extends React.Component<TextInputProps, State> {
  ...
  componentDidMount() {
    ...
    if (this.ref.current) {
      this.ref.current.xgeniaNode = this.props.xgeniaNode;
    }
  ...
  render() {
    ...
    inputStyles.color = props.xgeniaNode.context.styles.resolveColor(inputStyles.color);
    ...
    if (props.type !== 'textArea') {
      inputContent = (
        <input
          ref={(ref) => {
            if (ref) {
              this.ref.current = ref as InputRef;
            }
          }}
    ...
    const inputWithWrapper = (
      <div style={inputWrapperStyle} xgenia-style-tag="inputWrapper">
    ...
    if (props.useLabel) {
      ...
      return (
        <div style={otherStyles}>
          <label htmlFor={props.id} style={labelStyle} xgenia-style-tag="label">
            {props.label}
          </label>
          {inputWithWrapper}
        </div>
