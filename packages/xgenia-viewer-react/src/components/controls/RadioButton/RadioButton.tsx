...
export interface RadioButtonProps extends XGENIA.ReactProps {
  id: string;
  enabled: boolean;
  value: string;
...
export function RadioButton(props: RadioButtonProps) {
  ...
  const radioButton = (
    <div className="ndl-controls-pointer" style={inputWrapperStyle} xgenia-style-tag="radio">
      <div style={fillStyle} xgenia-style-tag="fill" />
      ...
      <input
        ...
        onChange={() => {
          // First notify the group so it updates selected value and propagates change
          const groupCheckedChanged = radioButtonGroup?.checkedChanged;
          if (groupCheckedChanged) {
            groupCheckedChanged(props.value);
          }
          // Then update this node's own checked state/output
          if (props.checkedChanged) {
            props.checkedChanged(true);
          }
        }}
      />
    </div>
