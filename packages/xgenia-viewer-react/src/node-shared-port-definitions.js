  ...
  addShadowInputs(definition, args) {
    ...
    definition.methods._updateBoxShadow = function () {
      ...
      if (internal.boxShadowEnabled) {
        this.setStyle(
          {
            boxShadow: `${internal.boxShadowInset ? 'inset ' : ''}${internal.boxShadowOffsetX} ${internal.boxShadowOffsetY
  ...
  addTextStyleInputs(definition, args) {
        ...
        set(value) {
          if (value) {
            ...
            if (family.split('.').length > 1) {
              // File-based font (e.g. MyFont.ttf)
    ...
    definition.setup = function (context, graphModel) {
      graphModel.on('nodeAdded.' + definition.name, function (node) {
        if (node.parameters[fontFamilyPort]) {
          var fontValue = node.parameters[fontFamilyPort];
          if (fontValue.split('.').length > 1) {
            // File-based font
