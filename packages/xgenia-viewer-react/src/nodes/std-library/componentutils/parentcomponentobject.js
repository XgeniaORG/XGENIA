...
const ParentComponentObject = {
  name: 'net.xgenia.ParentComponentObject',
  displayNodeName: 'Parent Component Object',
  category: 'Component Utilities',
  color: 'component',
  docs: 'https://docsapp.xgenia.com/nodes/component-utilities/parent-component-object',
  initialize() {
...
module.exports = {
  ...
  setup: function (context, graphModel) {
    ...
    graphModel.on('nodeAdded.net.xgenia.ParentComponentObject', (node) => {
    ...
    //use a setTimeout
    graphModel.on('nodeAdded.net.xgenia.ComponentObject', (node) => {
      setTimeout(() => {
        graphEventEmitter.emit('componentStateNodesChanged');
      }, 0);
    });
    graphModel.on('nodeRemoved.net.xgenia.ComponentObject', (node) => {
      setTimeout(() => {
        graphEventEmitter.emit('componentStateNodesChanged');
      });
    });
