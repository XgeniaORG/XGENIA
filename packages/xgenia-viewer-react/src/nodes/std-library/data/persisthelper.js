...
const { Services } = require('@xgenia/runtime');
...
PersistHelper.prototype.persistData = function (data) {
  ...
  if (persistType === 'local') {
    ...
    try {
      localStorage['xgenia-persist-' + id] = JSON.stringify(data);
      this.onPersistSuccess();
    } catch (e) {
      this.onPersistFailure();
    }
