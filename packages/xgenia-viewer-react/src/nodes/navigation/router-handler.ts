...
export class RouterHandler {
  ...
  getPagesForRouter(name) {
    const routerIndex = xgeniaRuntime.instance.graphModel.routerIndex;
  ...
  onNavigated(routerName: string, page: ComponentPageInfo) {
    // @ts-expect-error window.xgenia is not defined
    window.xgenia.Events.emit('xgeniaApp_Navigated', {
      routerName,
      ...page
    });
  }
