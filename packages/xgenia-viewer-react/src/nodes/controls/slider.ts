...
// Define the node with 'this' context for methods
const RangeNode = {
  name: 'net.xgenia.controls.range',
  displayNodeName: 'Slider',
  docs: 'https://docsapp.xgenia.com/nodes/ui-controls/slider',
  allowChildren: false,
  ...
  initialize(this: any) {
    ...
    this.props.value = this.props.min || 0;
    ...
    this.props.updateOutputValue(this.props.value || 0); // Set the value output to an initial value
  },
  getReactComponent() {
    return Slider;
  },
  ...
  methods: {
    _updateOutputValuePercent(this: any, value: number) {
      ...
      if (valuePercentChanged) this.flagOutputDirty('valuePercent');
    },
    _setInputValue(this: any, newValue) {
      //make sure value never goes out of range
      const min = this.props.min || 0;
      const max = this.props.max || 100;
      const value = Math.max(min, Math.min(max, newValue || 0));
...
export default createNodeFromReactComponent(RangeNode) as any;
