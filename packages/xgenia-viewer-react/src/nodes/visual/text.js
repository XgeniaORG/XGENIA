...
import { Text as TextComponent } from '../../components/visual/Text';
...
const TextNode = {
  name: 'Text',
  docs: 'https://docsapp.xgenia.com/nodes/basic-elements/text',
  visualStates: [
...
if (TextNode.methods) {
  for (const methodName in TextNode.methods) {
    if (typeof TextNode.methods[methodName] === 'function') {
      const originalMethod = TextNode.methods[methodName];
      TextNode.methods[methodName] = function (...args) {
...
if (TextNode.inputs) {
  for (const inputName in TextNode.inputs) {
    if (TextNode.inputs[inputName] && typeof TextNode.inputs[inputName].set === 'function') {
      const originalSet = TextNode.inputs[inputName].set;
      TextNode.inputs[inputName].set = function (...args) {
        return originalSet.apply(this, args);
      };
    }
    if (TextNode.inputs[inputName] && typeof TextNode.inputs[inputName].valueChangedToTrue === 'function') {
      const originalValueChangedToTrue = TextNode.inputs[inputName].valueChangedToTrue;
      TextNode.inputs[inputName].valueChangedToTrue = function (...args) {
