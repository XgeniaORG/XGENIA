...
export interface TreeViewProps<TNode extends TreeViewItem<TNode>> {
  items: TNode[];
  node?: React.ComponentType<TreeViewChildProps<TNode>>;
...
function TreeViewVisual<TNode extends TreeViewItem<TNode>>({ items, node, onItemClick }: TreeViewVisualProps<TNode>) {
  ...
  return (
    <div
      ...
      onDragStart={(eventObject) => {
        ...
        eventObject.dataTransfer.setData('xgenia/draggable-id', id);
      }}
      onDragEnd={() => {
        context.setDropSpot(null);
      }}
      onDragOver={(eventObject) => {
        if (!!eventObject.dataTransfer.getData('xgenia/draggable-id')) {
          return;
        }
      ...
      onDrop={(eventObject) => {
        ...
        const sourceId = eventObject.dataTransfer.getData('xgenia/draggable-id');
