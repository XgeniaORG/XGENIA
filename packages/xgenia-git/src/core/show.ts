...
export async function getBlobContents(repositoryDir: string, commitish: string, path: string): Promise<string> {
  ...
  const { output, exitCode, error } = await git(args, repositoryDir, 'getBlobContents', opts);
  if (exitCode !== 0) {
    throw new Error(error ? error.toString() : 'Failed to get blob contents');
  }
  return output;
}
...
export async function getBlobBinaryContents(repositoryDir: string, commitish: string, path: string): Promise<Buffer> {
  ...
  const { output, exitCode, error } = await git(args, repositoryDir, 'getBlobBinaryContents', opts);
  if (exitCode !== 0) {
    throw new Error(error ? error.toString() : 'Failed to get blob binary contents');
  }
