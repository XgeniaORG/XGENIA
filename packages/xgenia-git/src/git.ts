...
import { createBranch, currentBranchName } from './core/branch';
...
import { getMergeBase, merge, mergeTree, mergeTreeCommit } from './core/merge';
...
import { addRemote, getRemotes, getRemoteURL } from './core/remotes';
...
import { createStashEntry, getStashes, popStashEntry, popStashEntryToBranch } from './core/stash';
...
export type GitProvider = 'xgenia' | 'github' | 'unknown' | 'none';

/**
 * NOTE: Don't change the methods here, they are the same as in XGENIA Editor right now!
...
export class Git {
  ...
  async stashPushChanges(branchName?: string) {
    ...
    const stashMarker = `!!XGENIA<${useBranchName}>`;
  ...
  async stashPopChanges(branchName?: string): Promise<boolean> {
    ...
    try {
      ...
      // So we created a branch called !!XGENIA-Stash-... with all has our (local) changes
      // Then we merge in the remote changes to our new branch without any problems
      // So I checked out the branch we are working on
      // And the squash merge all the files from without a commit in our new !!XGENIA-Stash
      //
      ...
      if (err.toString().includes('could not restore untracked files from stash')) {
        ...
        const stashBranchName = `!!XGENIA-Stash-${stash.branchName}`;
  ...
  /**
   * Setup the common git rules for it to work with XGENIA.
   */
  private async _setupRepository() {
    ...
    await appendGitAttributes(this.baseDir, ['project.json merge=xgenia']);
  ...
  public getProviderForRemote(remoteUrl: string): GitProvider {
    if (!remoteUrl) {
      return 'none';
    } else if (remoteUrl.includes('xgeniaapp.com')) {
      return 'xgenia';
