    ...
    function walk(currentDir) {
        ...
        for (const item of items) {
            ...
            if (stat.isDirectory()) {
                ...
                // FIX (2026-04-24): Also walk .ts files so leaf node defs in
                // packages/xgenia-viewer-react/src/nodes/controls/*.ts
                // (button.ts, text-input.ts) get indexed. Prior to this only
                // .js files were considered → Button & TextInput were absent
                // from search-index.json and the validator had to fall back
                // to a hardcoded UI overlay.
