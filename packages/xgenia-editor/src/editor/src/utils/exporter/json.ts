...
import { CloudServiceMetadataDataFormat, ProjectModel } from '@xgenia-models/projectmodel';
...
export function exportComponentsToJSON(
  ...
  if (args?.environment === null) {
    ...
    const isSupabaseEnvironment = args.environment.isSupabase && args.environment.isSupabase();
    
    let endpointForBase = args.environment.url;
    if (isSupabaseEnvironment && args.environment.supabaseUrl) {
      endpointForBase = args.environment.supabaseUrl;
    }
    ...
    if (isSupabaseEnvironment) {
      json.metadata['cloudservices'] = {
        ...baseMetadata,
        supabase: {
          enabled: true,
          url: args.environment.supabaseUrl,
          anonKey: args.environment.anonKey,
          serviceRoleKey: args.environment.serviceRoleKey,
          enableRealtime: args.environment.enableRealtime !== false
        },
        routing: {
          default: 'supabase',
          collections: {}
      ...
      json.metadata['cloudservices'] = baseMetadata;
    }
  }
