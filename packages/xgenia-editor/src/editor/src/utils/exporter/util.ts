...
export function exportComponent(comp: ComponentModel) {
  ...
  // Guard against missing graph (can happen during component creation)
  if (!comp?.graph) {
    console.warn(`[exportComponent] Component "${comp?.name}" has no graph yet - returning empty structure`);
    return json;
  }
