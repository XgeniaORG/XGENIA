...
import { getPageRoutes } from '@xgenia-utils/compilation/context/pages';
...
let singletonViewerPkg: TypescriptModule | null = null;
let viewerInitPromise: Promise<void> | null = null;

export function GetOrCreateViewerModel(): TypescriptModule {
  // Return cached singleton if available
  if (singletonViewerPkg) {
    return singletonViewerPkg;
  }

  const libPathName = 'inmemory://@xgenia/viewer/global.d.ts';
  const libUri = monaco.Uri.parse(libPathName);

  const pkg = new TypescriptModule();
  singletonViewerPkg = pkg; // Cache immediately to prevent re-entry
  ...
  if (!viewerInitPromise) {
    viewerInitPromise = (async () => {
      try {
        ...
        console.warn('[GetOrCreateViewerModel] Failed to load types:', e);
      }
    })();
  }
