...
const { projectFromDirectory } = require('@xgenia-models/projectmodel.editor');
    ...
    importProject.listFilesInProjectDirectory(
      function (entries) {
        ...
        entries.forEach((e) => {
          ...
          // Skip .git
          else if (e.fullPath.startsWith(importProject._retainedProjectDirectory + '/xgenia_modules')) return; // Ignore modules
        ...
        let modules = [];
        try {
          const _modules = FileSystem.instance.readDirectorySync(direntry + '/xgenia_modules');
          modules = _modules
  ...
  ProjectModel.instance.listFilesInProjectDirectory((projectFiles) => {
    ...
    try {
      const _modules = FileSystem.instance.readDirectorySync(
        ProjectModel.instance._retainedProjectDirectory + '/xgenia_modules'
      );
      modules = _modules
        .filter((m) => FileSystem.instance.fileExistsSync(m.fullPath + '/manifest.json'))
        .map((m) => ({ name: m.name }));
        ...
        if (imports.modules !== undefined) {
          imports.modules.forEach((m) => {
            FileSystem.instance.copyRecursiveSync(
              importProject._retainedProjectDirectory + '/xgenia_modules/' + m.name,
              project._retainedProjectDirectory + '/xgenia_modules/' + m.name
            );
          });
        }
