...
export function windowTitleBarHeight() {
  let height = 34; //windows - matches industry standard title bar height

  if (process.platform === 'darwin') {
    const isBigSurOrAbove = require('os').release().split('.')[0] >= 20;
    height = isBigSurOrAbove ? 32 : 30;
  }
...
export function generateUniqueNodeLabel(baseLabel: string, existingLabels: Iterable<string>): string {
  const existing = new Set(existingLabels);
  let finalLabel = baseLabel;
  let counter = 2; // Start suffix at 2 (e.g., Group, Group 2)

  while (existing.has(finalLabel)) {
