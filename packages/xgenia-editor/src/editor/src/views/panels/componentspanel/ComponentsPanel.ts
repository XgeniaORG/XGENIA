...
import { HACK_findNodeReference } from '@xgenia-contexts/NodeReferencesContext';


// const PopupLayer = require('@xgenia-views/popuplayer');
import PopupLayer from '../../popuplayer';
import ComponentsPanelTemplate from '../../../templates/componentspanel.html';

// Styles
// require('../../../styles/componentspanel.css');
import '../../../styles/componentspanel.css';
...
export class ComponentsPanelView extends View {
  ...
  private getRuntimeType() {
    ...
    if (this._lockCurrentSheetName === '__maths__') {
      return 'maths';
    }

    const currentSheetName = this.currentSheet?.name;
    return currentSheetName === '#__cloud__' ? 'cloud' : currentSheetName === '#__maths__' ? 'maths' : 'browser';
  }
  ...
  render() {
    ...
    if (this._lockCurrentSheetName) {
      // Lock to a specific sheet
      this.currentSheet = (this.projectFolder?.folders || []).find((x) => x && x.name === '#' + this._lockCurrentSheetName);
      // Auto-create the sheet folder if it doesn't exist yet (e.g. __maths__ on first open)
      if (!this.currentSheet && this.projectFolder) {
        const newSheet = new ComponentsPanelFolder({
          name: '#' + this._lockCurrentSheetName,
          folders: [],
          components: []
        });
        this.projectFolder.addFolder(newSheet);
      ...
      if (c !== undefined && c.fullName) {
        ...
        if (newActiveSheet && newActiveSheet.name && !hiddenPrefix.some((x) => newActiveSheet.name.startsWith(x))) {
    ...
    EventDispatcher.instance.on(
      ...
      (args) => {
        ...
        if (_this._lockCurrentSheetName) {
          ...
          // Auto-create the sheet folder if it doesn't exist yet
          if (!_this.currentSheet && _this.projectFolder) {
            const newSheet = new ComponentsPanelFolder({
              name: '#' + this._lockCurrentSheetName,
              folders: [],
              components: []
            });
            _this.projectFolder.addFolder(newSheet);
            _this.currentSheet = newSheet;
          }
        }
  ...
  renderFolder(folder: ComponentsPanelFolder, appendTo: TSFixme, indent: number) {
    ...
    for (var i in folder.folders) {
      ...
      // Guard against undefined folder or name
      ...
      onCreate: (localName, options) => {
        const parentPath = scope.comp
          ...
          : scope.folder
            ...
            : _this.currentSheet?.getPath() || '/#' + (_this._lockCurrentSheetName || 'Default') + '/';
  ...
  onAddFolderClicked(scope, el, evt?) {
    ...
    const popup = new PopupLayer.StringInputPopup({
      ...
      onOk: function (folderName) {
        const result = _this.performAdd({
          type: 'folder',
          parentFolder: scope.folder ? scope.folder : (_this.currentSheet || _this.projectFolder),
          name: folderName
        });
        if (!result.success) ToastLayer.showError(result.message);
      }
  ...
  performAdd(args) {
    ...
    // Add a new folder
      ...
      const parentFolder = args.parentFolder ? args.parentFolder : (this.currentSheet || this.projectFolder);
  ...
  addComponentToFolderStructure(c, root?) {
    // Guard against undefined component or component without fullName
    if (!c) {
      console.warn('[ComponentsPanel] addComponentToFolderStructure called with null/undefined component');
      return;
    }

    if (!c.fullName || typeof c.fullName !== 'string') {
      console.warn('[ComponentsPanel] Component missing or invalid fullName:', {
        component: c,
        hasFullName: !!c.fullName,
        fullNameType: typeof c.fullName,
        fullNameValue: c.fullName
      });
      return;
    }

    // Guard against empty or whitespace-only names
    const trimmedName = c.fullName.trim();
    if (!trimmedName) {
      console.warn('[ComponentsPanel] Component has empty/whitespace fullName:', c.fullName);
      return;
    }
    ...
    function addToFolder(folder: ComponentsPanelFolder, path: string[], component: ComponentModel) {
      ...
      // Guard against undefined or empty path array
      if (!path || !Array.isArray(path) || path.length === 0) {
        console.warn(`[ComponentsPanel] addToFolder called with invalid path for component "${c?.fullName || 'unknown'}":`, {
          path,
      ...
      // Guard against invalid folder
      if (!folder) {
        console.warn(`[ComponentsPanel] addToFolder called with invalid folder for component "${c?.fullName || 'unknown'}":`, folder);
        return;
      }

      // Guard against invalid component
      if (!component || !component.fullName) {
        console.warn(`[ComponentsPanel] addToFolder called with invalid component:`, component);
        return;
      }
    ...
    if (!fullpath || typeof fullpath !== 'string') {
      console.warn(`[ComponentsPanel] Component has invalid fullName:`, c);
      return;
    }
    ...
    if (!fullpath) {
      console.warn(`[ComponentsPanel] Component fullName is empty after trimming:`, c.fullName);
      return;
    }

    if (fullpath[0] === '/') fullpath = fullpath.substring(1); // Ignore leading '/'
    const path = fullpath.split('/').filter(p => p && p.trim()); // Filter out empty strings
    if (path.length === 0) {
      console.warn(`[ComponentsPanel] Component "${c.fullName}" results in empty path after split. Original: "${c.fullName}", Processed: "${fullpath}"`);
      return;
    }
  ...
  getFolderStructure() {
    ...
    for (const index in components) {
      ...
      // Guard against undefined components or components without names
      if (!component || !component.fullName) continue;
  ...
  getSheetForComponentWithName(name: string) {
    // Guard against undefined or empty name
    if (!name) return this.projectFolder;

    // Sheet is simply to level folder
    const folders = name.split('/');
    if (folders[0] === '') folders.shift(); // Ignore leading /
    if (folders.length === 0 || folders.length === 1) return this.projectFolder;

    if (folders[0] && folders[0][0] === '#') return this.getFolderWithPath('/' + folders[0] + '/');
  ...
  getFolderWithPath(path: string, folder?) {
    ...
    if (!_folder || path === '/') return _folder;

    const folders = _folder.folders || [];
    for (const i in folders) {
      const f = folders[i];

      // Guard against undefined folder
      if (!f) continue;
