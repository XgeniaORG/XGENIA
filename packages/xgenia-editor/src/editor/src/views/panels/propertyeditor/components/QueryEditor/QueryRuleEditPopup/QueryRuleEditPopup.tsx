...
export class QueryRuleEditPopup extends React.Component<QueryRuleEditPopupProps> {
  ...
  onInputNameChanged(value) {
    // Ensure input is always stored as a string
    this.props.query.input = typeof value === 'string' ? value : String(value);
    this.props.onChange && this.props.onChange();
  }
  ...
  onSourceClicked() {
    ...
    if (q.input) {
      ...
      // Switch to input mode - ensure it's always a string
      q.input = 'MyInput';
      // Clear the value when switching to input mode
      q.value = undefined;
    }
    this.props.onChange && this.props.onChange();
  }
