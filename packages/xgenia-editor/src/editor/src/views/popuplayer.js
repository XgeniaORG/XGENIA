const KeyboardHandler = require('@xgenia-utils/keyboardhandler').default;
const { KeyCode } = require('@xgenia-utils/keyboard/KeyCode');
    ...
    if (ProjectModel.instance && isValid(evt.dataTransfer)) {
      ...
      function collectFiles(file, basedir) {
        if (FileSystem.instance.isPathDirectory(file.fullPath)) {
          ...
          // If basedir is null initially, use the directory of the current file for recursive calls
          const currentBaseDir = basedir === null ? FileSystem.instance.getFileDirectoryName(file.fullPath) : basedir;
          subfiles.forEach((f) => {
            // Ensure currentBaseDir is not null before calculating substring
            if (currentBaseDir !== null) {
                 collectFiles({ fullPath: f.fullPath, name: f.fullPath.substring(currentBaseDir.length + 1) }, currentBaseDir);
            } else {
                // Handle cases where directory couldn't be determined (should ideally not happen for directories)
                console.error("Could not determine base directory for recursive collectFiles");
                collectFiles({ fullPath: f.fullPath, name: f.name }, null); // Fallback using original name
            }
          });
      ...
      try {
        ...
        for (var i = 0; i < files.length; i++) {
          // Check if path exists and is a string before processing
          if (typeof files[i].path === 'string' && files[i].path.length > 0) {
             // Pass null instead of trying to get directory name from potentially undefined file.path
             collectFiles(
                { fullPath: files[i].path, name: files[i].name },
                 null
              );
          } else {
            // Handle files without a valid path (e.g., dragged from non-filesystem sources)
            skippedFiles++;
            console.warn(`Skipping file "${files[i].name}" due to missing path information.`);
          }
        }

        if (skippedFiles > 0) {
          const fileText = skippedFiles === 1 ? 'file' : 'files';
          ToastLayer.showError(`Could not process ${skippedFiles} ${fileText}: Path information unavailable. Ensure files are dragged from the file system.`);
        }

        if (_files.length > 0) { // Only proceed if there are files to copy
