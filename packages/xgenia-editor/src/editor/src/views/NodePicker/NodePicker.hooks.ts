import useCallAfterNextRender from '@xgenia-hooks/useCallAfterNextRender';
import useFetch from '@xgenia-hooks/useFetch';
import useOnMount from '@xgenia-hooks/useOnMount';
import { ipcRenderer } from 'electron';
import { Dispatch, RefObject, SetStateAction, useEffect, useReducer, useRef, useState } from 'react';

import { KeyCode } from '@xgenia-constants/KeyCode';
import { INodeType } from '@xgenia-types/nodeTypes';
import { DocsParser } from '@xgenia-utils/docs-parser';
...
export function useDocs() {
  ...
  function getDocs(type: INodeType) {
    timer.current = setTimeout(() => {
      ...
      docsUrl = docsUrl.replace('https://docsapp.xgenia.com', getDocsEndpoint());
...
// Safe storage helpers scoped to NodePicker news state
function safeGetNewsStateString(): string | null {
  try {
    return localStorage.getItem(NODE_PICKER_NEWS_STATE_STORAGE_KEY);
  } catch (err: any) {
    try {
      return sessionStorage.getItem(NODE_PICKER_NEWS_STATE_STORAGE_KEY);
    } catch {
      return null;
    }
  }
...
function safeSetNewsStateString(value: string): void {
  try {
    localStorage.setItem(NODE_PICKER_NEWS_STATE_STORAGE_KEY, value);
  } catch (err: any) {
    // Fallback to sessionStorage; if that fails, ignore
    try {
      sessionStorage.setItem(NODE_PICKER_NEWS_STATE_STORAGE_KEY, value);
    } catch {
      // no-op
    }
  }
...
function safeRemoveNewsState(): void {
  try {
  ...
  try {
    sessionStorage.removeItem(NODE_PICKER_NEWS_STATE_STORAGE_KEY);
  } catch {
    // no-op
  }
}
...
function getActiveIndex(items: SliderNews[]) {
  ...
  const storedData = safeGetNewsStateString();
  if (storedData) {
    try {
      const parsedData = JSON.parse(storedData);
      activeIndex = (parsedData?.activeIndex ?? 0) + 1;
      if (activeIndex >= items.length) {
        activeIndex = 0;
      }
    } catch {
      // ignore corrupt data and reset
      activeIndex = 0;
    }
  }

  safeSetNewsStateString(
    JSON.stringify({
      activeIndex
    })
  );
