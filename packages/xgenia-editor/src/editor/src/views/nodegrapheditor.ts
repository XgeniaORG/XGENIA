...
import { KeyCode } from '@xgenia-utils/keyboard/KeyCode';
...
import { PopupToolbar, PopupToolbarProps } from '@xgenia-core-ui/components/popups/PopupToolbar';
...
const log = debug('app:NodeGraphEditor');
...
export class NodeGraphEditor extends View {
  ...
  private trailRoot: Root | null = null;

  constructor(args) {
    super();

    log('Initializing Node Graph Editor with args: %O', args);
  ...
  reset() {
    ...
    // Clear the connections array to ensure clean state
    this.connections = [];
  ...
  bindModel(model?: NodeGraphModel) {
    console.log('[NodeGraphEditor] bindModel called with model instance:', model);
    const _this = this;
    ...
    model.on(
      ...
      (args) => {
        ...
        // Clean up any orphaned connections that reference this deleted node
    ...
    model.on(
      'connectionRemoved',
      function (args) {
        console.log('[NodeGraphEditor] connectionRemoved event triggered with args.model:', args.model, 'Current editor _this.model is:', _this.model);
        ...
        // Clean up any orphaned connections after removal
        _this.cleanupOrphanedConnections();
  ...
  insertNodeSet({
    ...
    // Ensure all pasted nodes have unique labels in the target graph
    const existingLabels = new Set<string>();
    this.model.forEachNode((n) => { if (n.label) existingLabels.add(n.label); });

    for (const root of ns.nodes) {
      root.forEach((node) => {
        const finalLabel = generateUniqueNodeLabel(node.label, existingLabels);
        if (node.label !== finalLabel) {
    ...
    function checkCreateStatus(nodes: NodeGraphNodeSet['nodes']) {
      for (const node of nodes) {
        if (node.type instanceof ComponentModel) {
          ...
          const effectiveRuntime = _this.runtimeType === RuntimeType.Maths ? RuntimeType.Browser : _this.runtimeType;
          if (node.type.runtimeTypes && !node.type.runtimeTypes.includes(effectiveRuntime)) {
  ...
  createNewNode(type: ComponentModel, pos: IVector2, options: Partial<NodeGraphNodeJSON> = {}) {
    const existingLabels = new Set<string>();
    this.model.forEachNode((n) => {
      if (n.label) existingLabels.add(n.label);
    });

    // Determine base label
    const baseLabel = options.label || type.displayName || type.name || 'UnknownNode';
    ...
    // Explicitly set the unique label
    node.label = finalLabel;
  ...
  updateTitle() {
    ...
    if (this.activeComponent) {
      ...
      const props = {
        componentTrail:
          this.model.owner && (this.model.owner as any).componentTrail ? (this.model.owner as any).componentTrail : [],
      ...
      // Create or reuse the root using createRoot
      if (!this.trailRoot) {
        this.trailRoot = createRoot(root);
      }
      this.trailRoot.render(React.createElement(NodeGraphComponentTrail, props));
    } else {
      // If there is no active component, unmount if we have a trailRoot
      if (this.trailRoot) {
        const rootToUnmount = this.trailRoot; // Capture the current value
        setTimeout(() => {
          // Check again inside setTimeout, as it could have been nulled by dispose()
          if (rootToUnmount) {
            rootToUnmount.unmount();
          }
          // Only nullify if it's the same one we intended to unmount and haven't already been nulled by dispose
          if (this.trailRoot === rootToUnmount) {
            this.trailRoot = null;
          }
        }, 0);
      }
  ...
  switchToComponent(
    ...
    console.log('[NodeGraphEditor] switchToComponent called:', {
      componentName: component?.name,
      componentId: component?.id,
      hasGraph: !!component?.graph,
      args: args
    });

    if (!component) {
      console.log('[NodeGraphEditor] No component provided, clearing active component');
    ...
    if (this.activeComponent !== component) {
      console.log('[NodeGraphEditor] Setting new active component:', {
        oldComponent: this.activeComponent?.name,
      ...
      console.log('[NodeGraphEditor] Calling bindModel with component.graph');
      this.bindModel(component.graph);

      this.commentLayer.setComponentModel(component);

      console.log('[NodeGraphEditor] Switch complete, activeComponent is now:', this.activeComponent?.name);
  ...
  selectNode(node: NodeGraphEditorNode) {
    ...
    if (!node.selected) {
      // Select node — keep right panel open to avoid flicker when switching nodes
      this.clearSelection({ disableHidePanels: true });
      this.commentLayer?.clearSelection();
      node.selected = true;
      ...
      setTimeout(() => {
        ...
        console.log("SidebarModel switched to new node:", node.model.id);
      }, 1);
      // SidebarModel.instance.switchToNode(node.model);
  ...
  setHighlightedNode(node: NodeGraphEditorNode, atPosition?) {
    // Fix: Prevent excessive setTimeout calls by checking if node actually changed
    if (node === this.highlighted) {
      return; // Same node, no need to do anything
    }
  ...
  setHighlightedConnection(c: NodeGraphEditorConnection, atPosition?) {
    // Fix: Prevent excessive setTimeout calls by checking if connection actually changed
    if (c === this.highlightedConnection) {
      return; // Same connection, no need to do anything
    }
    ...
    setTimeout(() => {
      ...
      const fromProps = {
        ...
        onPortSelected: (fromPort) => {
          console.log('onPortSelected called, this:', this);
          activePanel = 'to';
          // @ts-expect-error
          toProps.sourcePort = fromPort;
          toProps.disabled = false;
          // Update target popup using the toRoot instance
          toRoot.render(React.createElement(ConnectionPopup, toProps));

          fromProps.disabled = true;
          // Update source popup using the fromRoot instance
      ...
      const toProps = {
        ...
        onPortSelected: (toPort) => {
          activePanel = 'from';
          // Make the connection – this must be undoable
          // @ts-expect-error
  ...
  removeConnection(con) {
    // Find the NodeGraphEditorConnection object that wraps this model
    const editorConnection = this.connections.find(c =>
      c.model === con ||
      (c.fromId === con.fromId && c.toId === con.toId &&
        c.fromProperty === con.fromProperty && c.toProperty === con.toProperty)
    );

    // Call disconnect() on the editor connection to clean up node references
    if (editorConnection && typeof editorConnection.disconnect === 'function') {
      console.log('[NodeGraphEditor] Calling disconnect() on editor connection before model removal');
    ...
    // Also remove from our local connections array
    const index = this.connections.indexOf(editorConnection);
    if (index !== -1) {
      this.connections.splice(index, 1);
      console.log('[NodeGraphEditor] Removed connection from local connections array');
    }
  }
  ...
  showNodeToolbar(selectedNodes: readonly NodeGraphEditorNode[], aabb: AABB) {
    ...
    const toolbarRoot = createRoot(div);
    (div as any)._reactRoot = toolbarRoot;
    toolbarRoot.render(
  ...
  hideNodeToolbar() {
    ...
    for (const toolbar of toolbars) {
      const root = (toolbar as any)._reactRoot;
      if (root) root.unmount();
      if (this.domElementContainer.contains(toolbar)) {
  ...
  getContextMenuActions() {
    ...
    if (selectedNodes.length === 1 &&
      selectedNodes[0].model.typename === 'Image' &&
      selectedNodes[0].model.parameters.src
    ) {
      items.push({
        label: 'Edit Image with AI',
        onClick: () => {
          const node = selectedNodes[0];
          const src = node.model.parameters.src;
          let base64;
          ...
          EventDispatcher.instance.emit('openImageEditorWithImage', base64);
          SidebarModel.instance.switch('image-editor');
        },
  ...
  paint() {
    ...
    const viewMinY = Math.floor(-panAndScale.y / gridSize) * gridSize;
    ...
    _.each(this.connections, function (con) {
      // Safety check: skip connections with missing nodes
      if (con && con.fromNode && con.toNode) {
    ...
    _.each(this.connections, function (con) {
      // Safety check: skip connections with missing nodes
      if (con && con.fromNode && con.toNode && con.isHighlighted()) {
        con.paint(ctx, paintRect);
      }
    });
    ...
    if (this.draggingConnection) {
      ...
      if (this.draggingConnection.fromNode !== undefined && this.draggingConnection.toNode !== undefined) {
        ...
        _.each(this.connections, (con) => {
          // Safety check: ensure connection has valid nodes before comparing
          if (con && con.fromNode && con.toNode &&
            con.fromNode === this.draggingConnection.fromNode &&
            con.toNode === this.draggingConnection.toNode) {
            con.paint(ctx, paintRect);
          }
        });
      }
  ...
  /**
   * Clean up orphaned connections that reference non-existent nodes
   * This prevents crashes when the UI tries to paint connections with undefined nodes
   */
  cleanupOrphanedConnections() {
    ...
    for (const conn of this.connections) {
      if (conn && conn.model) {
        ...
        if (fromNode && toNode) {
          ...
          // Connection is orphaned
          console.warn('[NodeGraphEditor] Removing orphaned connection', {
            fromId: conn.model.fromId,
            toId: conn.model.toId,
    ...
    if (orphanedCount > 0) {
      console.log(`[NodeGraphEditor] Cleaned up ${orphanedCount} orphaned connections`);
