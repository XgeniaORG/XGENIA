...
function EditorDocument() {
  ...
  useEffect(() => {
    ...
    // --- Interactive editing: move element ---
    EventDispatcher.instance.on(
      'viewport-move-element',
      (args) => {
        const node = ProjectModel.instance.findNodeWithId(args.nodeId);
        if (!node) {
          console.warn('[EditorDocument] Move: node not found:', args.nodeId);
          return;
        }
        console.log('[EditorDocument] Moving node:', args.nodeId, 'delta:', args.deltaX, args.deltaY);
        console.log('[EditorDocument] Node type:', node.type, 'Current params:', JSON.stringify(node.parameters));

        // Use existing position param if found, otherwise default to marginLeft/marginTop
        const xParams = ['marginLeft', 'left', 'x', 'positionX'];
        const yParams = ['marginTop', 'top', 'y', 'positionY'];
        const xParam = xParams.find(p => node.parameters[p] !== undefined) || 'marginLeft';
        const yParam = yParams.find(p => node.parameters[p] !== undefined) || 'marginTop';

        const curX = parseFloat(node.parameters[xParam]) || 0;
        const curY = parseFloat(node.parameters[yParam]) || 0;

        const newX = Math.round(curX + args.deltaX);
        const newY = Math.round(curY + args.deltaY);

        // XGENIA nodes expect CSS strings with units (e.g. '15px'), not raw numbers
        node.setParameter(xParam, newX + 'px', { undo: true, label: 'move element' });
        node.setParameter(yParam, newY + 'px', { undo: true, label: 'move element' });

        console.log('[EditorDocument] Set', xParam, '=', newX + 'px', yParam, '=', newY + 'px');
      },
      eventGroup
    );

    // --- Interactive editing: resize element ---
    EventDispatcher.instance.on(
      'viewport-resize-element',
      (args) => {
        const node = ProjectModel.instance.findNodeWithId(args.nodeId);
        if (!node) {
          console.warn('[EditorDocument] Resize: node not found:', args.nodeId);
          return;
        }
        console.log('[EditorDocument] Resizing node:', args.nodeId, 'to:', args.width, '×', args.height);

        // Force sizeMode to 'explicit' so width/height are respected
        const currentSizeMode = node.parameters['sizeMode'];
        if (currentSizeMode && currentSizeMode !== 'explicit') {
          node.setParameter('sizeMode', 'explicit', { undo: true, label: 'resize element' });
        }

        // Use existing size param if found, otherwise default to width/height
        const wParam = ['width', 'sizeWidth', 'w'].find(p => node.parameters[p] !== undefined) || 'width';
        const hParam = ['height', 'sizeHeight', 'h'].find(p => node.parameters[p] !== undefined) || 'height';

        // XGENIA nodes expect CSS strings with units (e.g. '150px'), not raw numbers
        if (args.width != null) {
          node.setParameter(wParam, Math.round(args.width) + 'px', { undo: true, label: 'resize element' });
        }
        if (args.height != null) {
          node.setParameter(hParam, Math.round(args.height) + 'px', { undo: true, label: 'resize element' });
        }

        console.log('[EditorDocument] Set', wParam, '=', Math.round(args.width) + 'px', hParam, '=', Math.round(args.height) + 'px');
      },
      eventGroup
    );
