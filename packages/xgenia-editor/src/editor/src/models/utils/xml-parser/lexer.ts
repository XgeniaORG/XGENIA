...
export class Lexer {
  ...
  private readUntil(input: string, stopChar: string): string {
    ...
    // Debug logging for long tags
    if (value.length > 200 && value.includes('items=')) {
      console.log(`🔍 [Lexer] readUntil: Found long tag with items attribute`);
      console.log(`🔍 [Lexer] readUntil: Tag length: ${value.length}`);
      console.log(`🔍 [Lexer] readUntil: Tag starts with: "${value.substring(0, 50)}..."`);
      console.log(`🔍 [Lexer] readUntil: Tag ends with: "...${value.substring(value.length - 50)}"`);
    }
  ...
  private tokenizeTag(input: string): { tagName: string; attributes: Record<string, string> } {
    ...
    // If no space found, it's just a tag name
    if (spaceIndex === -1) {
      return { tagName: trimmed, attributes: {} };
    }

    const tagName = trimmed.substring(0, spaceIndex);
    const attributeString = trimmed.substring(spaceIndex + 1);
    const attributes: Record<string, string> = {};

    // Debug logging for complex attributes
    if (attributeString.includes('items=')) {
      console.log(`🔍 [Lexer] Processing tag: ${tagName}`);
      console.log(`🔍 [Lexer] Attribute string: "${attributeString}"`);
      console.log(`🔍 [Lexer] Attribute string length: ${attributeString.length}`);
    }

    // Robust attribute parsing that handles complex values like JSON strings
    let pos = 0;
    while (pos < attributeString.length) {
      // Skip whitespace
      while (pos < attributeString.length && /\s/.test(attributeString[pos])) {
        pos++;
      }
      ...
      const nameStart = pos;
      while (pos < attributeString.length && /[a-zA-Z0-9_]/.test(attributeString[pos])) {
        pos++;
      }
      const name = attributeString.substring(nameStart, pos);

      // Skip whitespace and equals sign
      while (pos < attributeString.length && /\s/.test(attributeString[pos])) {
        pos++;
      }
      if (pos >= attributeString.length || attributeString[pos] !== '=') break;
      ...
      // Parse the value
      let value = '';
      const quoteChar = attributeString[pos];
      ...
      if (quoteChar === '"' || quoteChar === "'") {
        ...
        while (pos < attributeString.length) {
          if (attributeString[pos] === quoteChar) {
            // Check if it's escaped
            if (pos > 0 && attributeString[pos - 1] === '\\') {
              // Escaped quote, continue
              pos++;
              continue;
            }
            // Found closing quote
            value = attributeString.substring(valueStart, pos);
        ...
        if (pos >= attributeString.length && !value) {
          // Incomplete quoted string, treat as unquoted
          pos = valueStart;
          while (pos < attributeString.length && !/\s/.test(attributeString[pos])) {
            pos++;
          }
      ...
      if (name) {
        ...
        // At this point the value has been safely extracted from the XML by
        // readUntil('>') and quote-matching, so decoding >, <, etc. is safe.
        attributes[name] = value
          .replace(/&quot;/g, '"')
          .replace(/&lt;/g, '<')
          .replace(/&gt;/g, '>')
          .replace(/&#39;/g, "'")
          .replace(/&amp;/g, '&'); // Must be last (after other &-patterns)

        // Debug logging for items attribute
        if (name === 'items') {
          console.log(`🔍 [Lexer] Parsed items attribute: "${attributes[name]}"`);
          console.log(`🔍 [Lexer] Items value length: ${attributes[name].length}`);
          console.log(`🔍 [Lexer] Items starts with: "${attributes[name].substring(0, 20)}..."`);
          console.log(`🔍 [Lexer] Items ends with: "...${attributes[name].substring(attributes[name].length - 20)}"`);
        }
      }
    }
