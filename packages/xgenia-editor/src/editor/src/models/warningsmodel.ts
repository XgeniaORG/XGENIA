...
// Don't import NodeLibrary during initialization to avoid circular deps
...
export class WarningsModel extends Model {
  ...
  constructor() {
    ...
    // Defer NodeLibrary binding until after initialization using setTimeout
    setTimeout(() => {
      // Import NodeLibrary only when needed
      const { NodeLibrary } = require('./nodelibrary');
