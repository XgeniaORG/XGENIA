...
export class NodeLibrary extends Model {
  ...
  loadLibrary() {
    ...
    console.log(window.NodeLibraryData);
    ...
    for (const i in this.library.nodetypes) {
      ...
      this.registerNodeType(new BasicNodeType(type));
    }
    ...
    if (!this.library.colors.connections.default) {
      this.library.colors.connections.default = {
        normal: 'rgba(103, 222, 146, 0.7)',
        highlighted: 'rgba(236, 91, 224, 0.9)',
        pulsing: 'rgba(255, 255, 255, 0.95)'
      };
    }
  }
  ...
  reload() {
    this.loadLibrary();
    // Always provide an args object.
    // Some listeners destructure callback args (e.g. `({ model }) => ...`) and will throw if undefined.
    this.notifyListeners('libraryUpdated', {} as any);
  }
  ...
  getNodeTypeWithName(typename: string): NodeLibraryNodeType {
    ...
    if (hasLoadedNodeLib && !this.typeCache.has(typename)) {
      const types = this.types.concat(this.getComponents());
      types.forEach((type) => {
        // Check if it's a BasicNodeType or ComponentModel
        const isComponentModel = type instanceof ComponentModel;

        // Set canBeChild with type safety
        if (isComponentModel) {
          // For ComponentModel
          if (typeof (type as ComponentModel).canBeChild === 'undefined') {
            // Default most components can be children
            (type as ComponentModel).canBeChild = true;

            // Only restrict these specific top-level node types
            // Limit this to only true top-level containers
            if (type.name === 'Project Root') {
              (type as ComponentModel).canBeChild = false;
            }
          }

          // Set allowChildren with type safety
          if (typeof (type as ComponentModel).allowChildren === 'undefined') {
            (type as ComponentModel).allowChildren = true;
          }
        } else {
          // For BasicNodeType or other types
          const basicNodeType = type as BasicNodeType;
          if (typeof basicNodeType.canBeChild === 'undefined') {
            basicNodeType.canBeChild = true;

            // Only restrict these specific top-level node types
            // Limit this to only true top-level containers
            if (basicNodeType.name === 'Project Root') {
              basicNodeType.canBeChild = false;
            }
          }

          if (typeof basicNodeType.allowChildren === 'undefined') {
            basicNodeType.allowChildren = true;
          }
        }
  ...
  registerNodeType(typeDef) {
    // Set default hierarchy behavior if not specified
    if (typeDef instanceof BasicNodeType) {
      // Apply default properties for hierarchy validation
      if (typeof typeDef.canBeChild === 'undefined') {
        // Default is that most components should be able to be children
        typeDef.canBeChild = true;
      }

      if (typeof typeDef.allowChildren === 'undefined') {
        // Default is that most components should be able to have children
        typeDef.allowChildren = true;
      }

      // Specific overrides for known top-level components
      if (typeDef.name === 'Project Root') {
        typeDef.canBeChild = false;
      }
    }
  ...
  colorSchemeForNodeColorName(name) {
    console.log(`[NodeLibrary] colorSchemeForNodeColorName looking for: '${name}'`);
    console.log(`[NodeLibrary] this.library.colors.nodes available:`, this.library?.colors?.nodes);
    const scheme = this.library?.colors?.nodes[name] || this.library?.colors?.nodes.default;
    console.log(`[NodeLibrary] colorSchemeForNodeColorName result:`, scheme);
    return scheme;
  }

  colorSchemeForNodeType(type) {
    console.log(`[NodeLibrary] colorSchemeForNodeType called for type:`, type);
    const scheme = this.colorSchemeForNodeColorName(type.color);
    console.log(`[NodeLibrary] colorSchemeForNodeType returning:`, scheme);
    return scheme;
  }
