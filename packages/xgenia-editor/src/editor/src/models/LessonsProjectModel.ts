    ...
    FileSystem.instance.makeDirectory(path, (r) => {
      ...
      FileSystem.instance.isDirectoryEmpty(path, (isEmpty) => {
        if (isEmpty) this._cloneLessonIntoDirectory(lesson, path, fn);
          ...
          if (isEmpty) {
            console.log('[LessonsProjectModel] Directory is empty, creating new lesson project');
            this._cloneLessonIntoDirectory(lesson, path, fn);
          } else {
            console.log('[LessonsProjectModel] Directory exists, loading existing project');
            // Lesson is started
            projectFromDirectory(path, (project) => {
              if (!project) {
                console.log('[LessonsProjectModel] Failed to load project from directory');
                return;
              }
              
              console.log('[LessonsProjectModel] Project loaded from directory, checking lesson property');
              console.log('[LessonsProjectModel] Project has lesson property:', !!project.lesson);
              
              // Ensure the lesson property is set even when loading from existing directory
              if (!project.lesson) {
                console.log('[LessonsProjectModel] Adding lesson property to existing project');
                const baseURL = lesson.url.split('/').slice(0, -1).join('/') + '/';
                project.lesson = new LessonModel({
                  baseURL: baseURL,
                  index: 0,
                  url: 'lesson.html',
                  name: lesson.name
                });
                console.log('[LessonsProjectModel] Lesson model added to project:', project.lesson);
              } else {
                console.log('[LessonsProjectModel] Project already has lesson property');
              }
              
              project.id = guid();
              this._trackLessonProgress(project, lesson.name);
              console.log('[LessonsProjectModel] Final project lesson property:', !!project.lesson);
              fn(project);
            });
          }
        }
