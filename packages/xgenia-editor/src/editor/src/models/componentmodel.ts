...
import Utils from '@xgenia-utils/utils';
...
// Cycle guard: prevents infinite recursion in allowAsChild/allowAsExportRoot/color/category
// when ComponentModels reference each other through graph.roots
const _traversing = new Set<ComponentModel>();
...
export class ComponentModel extends Model {
  ...
  get category() {
    if (_traversing.has(this)) return undefined;
    _traversing.add(this);
    try {
      let category;
      ...
      return category;
    } finally { _traversing.delete(this); }
  }

  get color() {
    if (_traversing.has(this)) return 'default';
    _traversing.add(this);
    try {
      let color;
  ...
  get allowAsChild() {
    if (_traversing.has(this)) return false;
    _traversing.add(this);
    try {
